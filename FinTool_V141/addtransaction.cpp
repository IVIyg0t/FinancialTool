    ...
    ui(new Ui::addTransaction)
{
    ...
    this->Categories = (QStringList() <<(QString)"" << (QString)"Add Category");
    this->Transaction_types = (QStringList() << (QString)"" << (QString)"Add Transaction Type");
...
bool addTransaction::eventFilter(QObject *target, QEvent *event){
    QString newType;
    if(target == ui->cb_account || target == ui->cb_category || target == ui->cb_transaction){
             ...
             QKeyEvent *keyEvent = static_cast<QKeyEvent *>(event);
                if((keyEvent->key() == Qt::Key_Enter) || (keyEvent->key() == Qt::Key_Return)){
                    ...
                    if(target == ui->cb_category){
                        newType =  ui->cb_category->currentText();
                        this->Categories += newType;
                        ui->cb_category->insertItem(ui->cb_category->findText("Add Category"), newType);
                        ui->cb_category->setCurrentIndex(ui->cb_category->findText(newType));
